...
const CountdownClock = () => {
  ...
  const ntpSyncIntervalRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const syncWithNTP = async () => {
    ...
    try {
      ...
      const drift = Math.abs(offset - ntpOffset);
      ...
      addDebugLog('API', 'NTP sync completed', { offset, server: ntpServer, drift });
      console.log('NTP sync completed. Offset:', offset, 'ms, Drift:', drift, 'ms');
    } catch (error) {
  ...
  useEffect(() => {
    ...
    ntpSyncIntervalRef.current = setInterval(syncWithNTP, 1800000);
    return () => {
      if (ntpSyncIntervalRef.current) {
        clearInterval(ntpSyncIntervalRef.current);
      }
    };
  }, [ntpServer]);
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-pink-400 mb-2">Bitfocus Companion Setup</h3>
                    <div className="bg-gray-900 p-4 rounded text-sm space-y-3">
                      <div>
                        <h4 className="text-white font-medium mb-2">1. Install Generic HTTP Module</h4>
                        <p className="text-gray-300">In Companion, add a new connection and search for "Generic HTTP" module.</p>
                      </div>
                      
                      <div>
                        <h4 className="text-white font-medium mb-2">2. Configure Connection</h4>
                        <ul className="list-disc list-inside space-y-1 text-gray-400">
                          <li>Target IP: <code className="bg-gray-800 px-1 rounded">{ipAddress}</code></li>
                          <li>Target Port: <code className="bg-gray-800 px-1 rounded">{window.location.port || 8080}</code></li>
                          <li>Default Protocol: <code className="bg-gray-800 px-1 rounded">http</code></li>
                        </ul>
                      </div>

                      <div>
                        <h4 className="text-white font-medium mb-2">3. Button Actions</h4>
                        <div className="space-y-2">
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-green-300 font-medium">Start Timer</div>
                            <div className="text-xs text-gray-400">Method: POST, Path: /api/start</div>
                          </div>
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-yellow-300 font-medium">Pause/Resume</div>
                            <div className="text-xs text-gray-400">Method: POST, Path: /api/pause</div>
                          </div>
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-red-300 font-medium">Reset Timer</div>
                            <div className="text-xs text-gray-400">Method: POST, Path: /api/reset</div>
                          </div>
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-blue-300 font-medium">Next Round</div>
                            <div className="text-xs text-gray-400">Method: POST, Path: /api/next-round</div>
                          </div>
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-purple-300 font-medium">Previous Round</div>
                            <div className="text-xs text-gray-400">Method: POST, Path: /api/previous-round</div>
                          </div>
                        </div>
                      </div>

                      <div>
                        <h4 className="text-white font-medium mb-2">4. Feedback Variables</h4>
                        <p className="text-gray-300 mb-2">Use GET requests to <code className="bg-gray-800 px-1 rounded">/api/status</code> for button feedback:</p>
                        <ul className="list-disc list-inside space-y-1 text-gray-400 text-xs">
                          <li>Timer status (running/paused/stopped)</li>
                          <li>Current time remaining</li>
                      ...
                      <div>
                        <h4 className="text-white font-medium mb-2">5. Advanced Configuration</h4>
                        <div className="space-y-2 text-xs">
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-white">Set Custom Time:</div>
                            <div className="text-gray-400">POST /api/set-time</div>
                            <div className="text-gray-400">Body: {"{"}"minutes": 10, "seconds": 0{"}"}</div>
                          </div>
                          <div className="bg-gray-800 p-2 rounded">
                            <div className="text-white">Set Rounds:</div>
                            <div className="text-gray-400">POST /api/set-rounds</div>
                            <div className="text-gray-400">Body: {"{"}"rounds": 5{"}"}</div>
                          </div>
                        </div>
                      </div>
                    </div>
